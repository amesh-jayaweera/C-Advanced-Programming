...
block_node *add_block(rsize_t block_size, void *list_top, rsize_t data_size, void *data_start)
{
	...
	Boolean success = 0 == block_size % BLOCK_ALIGNMENT && NULL != list_top && 0 < data_size && NULL != data_start && NULL != new_block;
...
void *first_fit(rsize_t block_size, rsize_t data_size, void *data_start)
{
	...
	Boolean success = 0 == block_size % BLOCK_ALIGNMENT && 0 < block_size && 0 < data_size && NULL != data_start;

	if (success)
	{
		prev_size = (unsigned int)data_start;

		while (traverse_block != NULL && ((unsigned int)traverse_block->block_start - prev_size < block_size))
		{
			prev_block = traverse_block;
			traverse_block = traverse_block->next;
			prev_size = (unsigned int) prev_block->block_start + (unsigned int)prev_block->size;
		}
